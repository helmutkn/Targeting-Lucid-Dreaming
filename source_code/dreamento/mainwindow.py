...
from scripts import periodogram, realTimeAutoScoring
from scripts.RecorderThread import RecordThread
from scripts.ZmaxHeadband import ZmaxHeadband
from scripts.lspopt_all_in_one import spectrogram_lspopt  # for spectrogram plotting

from mplwidget import MplWidget
from scripts.Plotter import CustomWidget
...
class Window:
    def __init__(self):
        ...
        self.dlg = uic.loadUi(os.path.join(RELATIVE_PATH, 'resources', "mainWindows.ui"))
        ...
        self.dlg.signalTypeComboBox.addItems(
            ["EEGR", "EEGL", "TEMP", "EEGR, EEGL", "DX, DY, DZ", "EEGR, EEGL, TEMP", "EEGR, EEGL, TEMP, DX, DY, DZ"])
        ...
        self.dlg.scoreSleepCheckBox.stateChanged.connect(
            lambda x: self.scoreSleepCheckBoxEnabled() if x else self.scoreSleepCheckBoxDisabled())
        ...
        self.eegLine1 = self.dlg.graphWidget.plot(t, np.random.randn(30 * self.sample_rate), self.EEGLinePen1)
        self.eegLine2 = self.dlg.graphWidget.plot(t, np.random.randn(30 * self.sample_rate), self.EEGLinePen2)
        ...
        self.sleepScoringMethods = ["LightGBM", #"CNN + LSTM" was removed due to incompatible tf version
                                    "SVM"]  # CAUTION: use same words in recording thread as well!
    ...
    def triggerLightClicked(self):
        ...
        if self.isRecording:
            ...
            self.stimulationDataBase[
                f"LIGHT sample {stimulationSampleNum}, second {stimulationSecondNum}, totalSample {stimulationTotalSampleNum}"] = \
                f"""{color}, \
...
Vib: {self.dlg.vibrationBox.isChecked()}, Alt: {self.dlg.altEyesBox.isChecked()}"""
    ...
    def triggerSoundClicked(self):
        if self.audio_file_name != "":
            ...
            if self.isRecording:
                ...
                self.stimulationDataBase[
                    f"SOUND sample {stimulationSampleNum}, second {stimulationSecondNum}, totalSample {stimulationTotalSampleNum}"] = \
        ...
        if self.dlg.textToSpeechLineEdit.text() != "":
            try:
                ...
                if self.isRecording:  # if a recording is in progress...
                    ...
                    stimulationSampleNum, stimulationSecondNum, stimulationTotalSampleNum = self.recordingThread.getCurrentSampleInformation()
                    self.stimulationDataBase[
                        f"TEXT2SPEECH sample {stimulationSampleNum}, second {stimulationSecondNum}, totalSample {stimulationTotalSampleNum}"] = \
    ...
    def recordClicked(self):
        if self.isRecording is False:
            ...
            if self.firstRecording:
                if self.dlg.scoreSleepCheckBox.isChecked() and \
                    ...
                    print('This model is not available in the current version!')
            ...
            self.dlg.recordButton.setIcon(QIcon(os.path.join(RELATIVE_PATH, 'resources', 'graphics/record.png')))
            ...
            self.dlg.recordButton.setIcon(QIcon(os.path.join(RELATIVE_PATH, 'resources', 'graphics/record_gray.png')))
        ...
        self.recordingThread.sendEEGdata2MainWindow.connect(
            self.getEEG_from_thread)  # sending data to mainWindow for plotting, scoring, etc.
        # self.dlg.graphWidget.clear()
    ...
    def displayEpochPredictionResult(self, predResult, epochNum):
        ...
        self.dlg.appDisplay.appendHtml(
            f"<font style='color:{stagesListColor[predResult]};' size='4'>{epochNum:03}. {stagesList[predResult]}</font>")
        ...
        with open(f"{fileName}-predictions.txt", "a") as outfile:
            if self.scoring_predictions != []:
                ...
                self.scoring_predictions.insert(0,
                                                -1)  # TODO (what?!) first epoch is not predicted, therefore put -1 instead
    ...
    def setMarkerButtonPressed(self):
        if self.isRecording:  # if a recording is in progress...
            ...
            stimulationSampleNum, stimulationSecondNum, stimulationTotalSampleNum = self.recordingThread.getCurrentSampleInformation()
            self.stimulationDataBase[
                f"MARKER sample {stimulationSampleNum}, second {stimulationSecondNum}, totalSample {stimulationTotalSampleNum}"] = \
    ...
    def getEEG_from_thread(self, eegSignal_r, eegSignal_l,
        ...
        if plot_periodogram or plot_spectrogram:
            if eegSignal_r is None:
                ...
                data = np.asarray(
                    eegSignal_l)  # choose left one (no reason for this) because only one side is enought, while coder added l & r.
            ...
            if self.dlg.scoreSleepCheckBox.isChecked():
                if algorithm == "CNN + LSTM":
                    if self.sleepScoringModel is not None:
                        # 30 seconds, each 128(256) samples... send recording for last 30 seconds to model for prediction
                        ...
                        modelPrediction = realTimeAutoScoring.Predict_array(
                            ...
                            lowcut=0.3, highcut=30, fs=self.sample_rate, signal_req=sigReq, signal_ref=sigRef,
                            model=self.sleepScoringModel)
